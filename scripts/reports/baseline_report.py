...
ANALYTIC_RESULTS_PATH = ROOT / "experiments" / "analytic_connections" / "analytic_results.json"


def load_empirical_signal() -> Dict[int, float]:
    """Build a real-world-like signal from the saved cognitive load traces."""
    signal_files = [
        DATA_SIMULATED_DIR / "results_load_0.csv",
        DATA_SIMULATED_DIR / "results_load_30.csv",
        DATA_SIMULATED_DIR / "results_load_70.csv",
    ]
...
def load_threshold_protocol() -> List[Dict[str, float]]:
    ...
    path = DATA_REFERENCE_DIR / "threshold_map.csv"
    if not path.exists():
        return []
...
def save_reports(seed_report: Dict, holdout_report: Dict, ablation_report: Dict):
    ...
    print(f"\n✓ Reports saved to: {report_path.relative_to(ROOT)}")
